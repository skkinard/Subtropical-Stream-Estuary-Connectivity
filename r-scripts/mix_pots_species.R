...
# Assign sites
my_sites <-  d %>% filter(SITE_TYPE == 'Fresh') %>%
  pull(SITE) %>%
  unique() %>%
  as.character() %>%
  sort()

# Estuary-influenced Potamodromous species:
my_species <- d %>% 
  filter(SITE_TYPE == 'Fresh') %>%
  filter(GUILD %in% c('Fish', 'Invertebrate')) %>%
  filter(CARBON > -20 | SULFUR > 10) %>%
...
mix_site_species <- left_join(my_table, df%>%select(SITE, BAYDIST_KM:ELEV_SITE_M)%>%unique())
...
# Plots
( species_PPTAVG_SITE <- plotter(filter(d, TAXON %in% my_sp2), PPTAVG_SITE) +
    xlab('Rainfall (cm/yr)') )

( species_BAYDIST_KM <- plotter(filter(d, TAXON %in% my_sp2), BAYDIST_KM) +
    xlab('Estuary Distance (km)') )

( species_ELEV_SITE_M <- plotter(filter(d, TAXON %in% my_sp2), ELEV_SITE_M) +
    xlab('Elevation (m)') )

( species_HIRES_LENTIC_DENS <- plotter(filter(d, TAXON %in% my_sp2), HIRES_LENTIC_DENS) +
    xlab('Lentic Density') )

( species_NPDES_MAJ_DENS <- plotter(filter(d, TAXON %in% my_sp2), NPDES_MAJ_DENS) +
    xlab('Wastewater Density') )

( species_RAW_DIS_NEAREST_MAJ_NPDES <- plotter(filter(d, TAXON %in% my_sp2), RAW_DIS_NEAREST_MAJ_NPDES) +
    xlab('Wastewater Distance') )

( species_RAW_DIS_NEAREST_DAM <- plotter(filter(d, TAXON %in% my_sp2), RAW_DIS_NEAREST_DAM) +
    xlab('Dam Distance') )
...
# Export plots
ggsave('Figures/species_PPTAVG_SITE.png',
       species_PPTAVG_SITE,
       'png')
ggsave('Figures/species_BAYDIST_KM.png',
       species_BAYDIST_KM,
       'png')
ggsave('Figures/species_ELEV_SITE_M.png',
       species_ELEV_SITE_M,
       'png')
ggsave('Figures/species_HIRES_LENTIC_DENS.png',
       species_HIRES_LENTIC_DENS,
       'png')
ggsave('Figures/species_NPDES_MAJ_DENS.png',
       species_NPDES_MAJ_DENS,
       'png')
ggsave('Figures/species_RAW_DIS_NEAREST_MAJ_NPDES.png',
       species_RAW_DIS_NEAREST_MAJ_NPDES,
       'png')
ggsave('Figures/species_RAW_DIS_NEAREST_DAM.png',
       species_RAW_DIS_NEAREST_DAM,
       'png')