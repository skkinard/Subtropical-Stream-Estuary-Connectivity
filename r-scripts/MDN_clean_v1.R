...
miso <- miso %>%
  mutate(TAXON = ifelse(GUILD == 'Fish',
                 ...
                 ifelse(GUILD == 'Invertebrate',
                        FAMILY,
                 SOURCE_TYPE))) %>%
  mutate(TAXON = ifelse(str_detect(UID, 'macrobrachium'),
                        'Macrobrachium',
                        TAXON)) %>%
  mutate(TAXON = ifelse(TAXON == 'mix_tv',
                        'Riparian', 
                 ifelse(TAXON == 'filamentous_algae',
                        'algae', TAXON))) %>%
...
miso <- miso %>% 
  mutate(SITE_TYPE = ifelse(SITE %in% c('BB', 'CB', 'HB', 'LB', 'NB'), 'Estuary', 'Fresh'))
...
mmigrants <- c("A.mitchilli", "A.spatula", "C.variegatus", "D.maculatus" , "F.grandis", "G.dormitor", "M.berylina", "M.cephalus", "Macrobrachium", "Panopeidae")
...
write_csv(miso, "data/MDN_clean.csv")
